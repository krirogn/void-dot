...
volume() {
	mute=$(amixer sget Master | grep 'Front Left:' | awk -F'[][]' '{ print $4 }')

	if [ "$mute" = "on" ]; then
    printf "^c#b881c1^  "
    printf "^c#b881c1^ $(amixer sget Master | grep 'Front Left:' | awk -F'[][]' '{ print $2 }')"
	else
    printf "^c#b881c1^  "
	fi
}
...
clock() {
  ...
  printf "^c#1e222a^^b#7aa2f7^ $(date '+%A, %H:%M %p') "
}

while true; do

  [ $interval == 0 ] || [ $(($interval % 3600)) == 0 ] && updates=$(xbps-install -un | wc -l)
  interval=$((interval + 1))

  sleep 1 && xsetroot -name "$(update_icon) $updates $(batt) $(brightness) $(volume) $(cpu) $(mem) $(wlan) $(clock)"
done
